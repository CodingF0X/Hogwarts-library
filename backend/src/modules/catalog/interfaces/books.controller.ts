...
import { IDeleteBookService } from '../applications/ports/book/delete-book.service.interface';
...
export class BooksController {
  constructor(
    ...
    @Inject(BOOK_TOKEN.SERVICES.DELETE)
    private readonly deleteBookSVC: IDeleteBookService,
  ) {}
  ...
  async deleteBook(@Param('id') id: number): Promise<string> {
    return await this.deleteBookSVC.delete(id).catch((err) => {
      throw new HttpException(err.message, HttpStatus.BAD_REQUEST);
    });
  }
}
